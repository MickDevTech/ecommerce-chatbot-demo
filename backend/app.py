...
def classify_question_intent(question: str, pipe) -> Dict[str, Any]:
    ...
    classification_prompt = (
        ...
        f"Tipos de pregunta:\n"
        f"- 'categorias_disponibles': pregunta sobre QUÉ categorías existen (ej: 'qué categorías tienes', 'qué tipos hay', 'cuáles son las categorías')\n"
        f"- 'categoria': pregunta pidiendo VER productos de UNA categoría específica (ej: 'muéstrame electrónica', 'quiero ver ropa', 'productos de calzado')\n"
        f"- 'producto_especifico': pregunta sobre UN producto en particular (ej: 'laptop 14', 'botines de cuero')\n"
        f"- 'general': pregunta general sobre el catálogo (ej: 'qué vendes', 'qué tienes', 'recomiéndame')\n"
        f"- 'fuera_catalogo': pregunta que NO es sobre productos (ej: 'hola', 'cómo estás', 'horarios')\n\n"
        f"IMPORTANTE: Si mencionan una categoría específica (ropa, electrónica, calzado, accesorios), usa 'categoria', NO 'categorias_disponibles'.\n\n"
        f"Responde SOLO con este formato JSON:\n"
        f"{{\"tipo\": \"tipo_de_pregunta\", \"terminos\": [\"palabra1\", \"palabra2\"], \"categoria\": \"categoria_si_aplica\"}}\n"
        f"Ejemplo 1: Pregunta: 'Qué categorías tienes?' -> {{\"tipo\": \"categorias_disponibles\", \"terminos\": [], \"categoria\": null}}\n"
        f"Ejemplo 2: Pregunta: 'Muéstrame electrónica' -> {{\"tipo\": \"categoria\", \"terminos\": [\"electronica\"], \"categoria\": \"electrónica\"}}\n"
        f"Ejemplo 3: Pregunta: 'Tienes camisetas?' -> {{\"tipo\": \"categoria\", \"terminos\": [\"camiseta\"], \"categoria\": \"ropa\"}}\n"
        f"Ejemplo 4: Pregunta: 'Dame info de laptop 14' -> {{\"tipo\": \"producto_especifico\", \"terminos\": [\"laptop\", \"14\"], \"categoria\": \"electrónica\"}}\n"
        f"Ejemplo 5: Pregunta: 'Hola' -> {{\"tipo\": \"fuera_catalogo\", \"terminos\": [], \"categoria\": null}}<|im_end|>\n"
        f"<|im_start|>user\n"
